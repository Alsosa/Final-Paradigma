...
def formCargarVenta():
    ...
    if formulario.validate_on_submit():
        ...
        for caracter in datoDeCodigo:
            if cont <= 3:
                if caracter.isdigit():
                    return render_template('cargar-venta/codigoerror.html')
                    cont = cont + 1
            elif cont >= 4:
                if caracter.isalpha():
                    return render_template('cargar-venta/codigoerror.html')
            cont = cont + 1
        datosIngresados = [datoDeCodigo, datoDeProducto, datoDeCliente, datoDeCantidad, datoDePrecio]
        resultado = funciones.escrituraDeVentas(datosIngresados)
        return render_template('lista_completa.html', ventas=funciones.lecturaDeVentas())
  
    return render_template('cargar-venta/formulario_cargar_venta.html', cargarVenta=datosIngresados, form=formulario)
