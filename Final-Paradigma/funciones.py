...
def escrituraDeVentas(datosIngresados):

    try:
        with open("examen.csv", mode='a') as archivo:
            archivo_writer = csv.writer(archivo, delimiter=",", lineterminator='\n')
            archivo_writer.writerow(datosIngresados)

    except:
        datosIngresados.append(["Error al cargar venta."])
